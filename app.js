...
function renderTimelineDashboard(entries) {
  ...
  if ('__show_cardio::2' in itemTimeMap) {
    allBlocks.push({ type: 'cardio', tlKey: 'cardio::session::2', sentinelKey: '__show_cardio::2', sessionIdx: 2, intraSlot: INTRA_CARDIO_SLOT_2, meal: null });
  }
  ...
  for (const sess of trainingSessions) {
    ...
    allBlocks.forEach(block => {
      ...
      if ((itemTimeMap[block.tlKey] ?? null) !== sess.block.intraSlot) return;
      if (block.type === 'item') {
        const e = block.entry;
        wm.kcal += e.kcal||0; wm.p += parseFloat(e.protein)||0;
        wm.c += parseFloat(e.carbs)||0; wm.f += parseFloat(e.fat)||0;
      } else {
        const pm = macroSum(block.entries); const d = block.isExploded ? 1 : block.servings;
        wm.kcal += pm.kcal/d; wm.p += pm.p/d; wm.c += pm.c/d; wm.f += pm.f/d;
      }
    });
    sess.block.windowMacros = wm;
  }
  ...
  for (const sess of insulinSessions) {
    ...
    allBlocks.forEach(block => {
      ...
      const m = itemTimeMap[block.tlKey] ?? null;
      if (m == null || m < sess.slot || m > endSlot) return;
      if (block.type === 'item') {
        const e = block.entry;
        wm.kcal += e.kcal||0; wm.p += parseFloat(e.protein)||0;
        wm.c += parseFloat(e.carbs)||0; wm.f += parseFloat(e.fat)||0;
  ...
  const nullBlocks = bySlot['null'] || [];
  {
    ...
    // Training button — 0: duration modal; 1–2: action modal
    {
      ...
      btn.type = 'button';
      btn.className = 'tl-chip-add-btn' + (cnt > 0 ? ' active' : '');
      btn.innerHTML = `<i class="fas fa-dumbbell"></i>` + (cnt > 1 ? `<span class="tl-chip-count">${cnt}</span>` : '');
      btn.addEventListener('click', () => {
        if (cnt === 0) {
    ...
    // Cardio button — 0: duration modal; 1–2: action modal
    {
      ...
      btn.type = 'button';
      btn.className = 'tl-chip-add-btn' + (cnt > 0 ? ' active' : '');
      btn.innerHTML = `<i class="fas fa-person-running"></i>` + (cnt > 1 ? `<span class="tl-chip-count">${cnt}</span>` : '');
      btn.addEventListener('click', () => {
        if (cnt === 0) {
  ...
  // Add Intra Workout/Cardio rows only when the respective session chip is placed.
  for (const sess of trainingSessions) {
    if (sess.slot != null) wrap.appendChild(buildTlRow(sess.block.intraSlot, bySlot[sess.block.intraSlot] || []));
  }
  for (const sess of cardioSessions) {
    if (sess.slot != null) wrap.appendChild(buildTlRow(sess.block.intraSlot, bySlot[sess.block.intraSlot] || []));
  }
  ...
  for (const sess of insulinSessions) {
    ...
    const insulinRow = wrap.querySelector(`[data-hour="${sess.slot}"]`);
  ...
  for (const sess of trainingSessions) {
    if (sess.slot == null) continue;
    const slots = Math.floor(sess.duration / 30);
    const wm = sess.block.windowMacros;

    for (let i = 0; i < slots; i++) {
      const coveredRow = wrap.querySelector(`[data-hour="${sess.slot + i * 30}"]`);
      if (coveredRow) coveredRow.classList.add('tl-training-covered');
    }

    const trainingRow = wrap.querySelector(`[data-hour="${sess.slot}"]`);
    const block = document.createElement('div');
    ...
    const intraRow = wrap.querySelector(`[data-hour="${sess.block.intraSlot}"]`);
  ...
  // Build cardio blocks (chip-bar + covered rows + Intra Cardio row + footer)
  for (const sess of cardioSessions) {
    if (sess.slot == null) continue;
    const slots = Math.floor(sess.duration / 30);
    const wm = sess.block.windowMacros;

    const cardioRow = wrap.querySelector(`[data-hour="${sess.slot}"]`);
    const block = document.createElement('div');
...
function refreshMealRail(dragActive) {
  ...
  const inBlockRows = new Set([
    ...
    ...wrap.querySelectorAll('.tl-cardio-block .tl-row'),
  ]);
  const measured = [];
...
function buildTlRow(minutes, blocks) {
  ...
  lbl.textContent = isNull ? '–'
    : (minutes === INTRA_WORKOUT_SLOT || minutes === INTRA_WORKOUT_SLOT_2) ? 'Intra Workout'
    : (minutes === INTRA_CARDIO_SLOT  || minutes === INTRA_CARDIO_SLOT_2)  ? 'Intra Cardio'
    : formatSlot(minutes);
  ...
  if (settings.showSlotTotals && foodBlocks.length >= 2 && !isIntraSlot) {
    const total = foodBlocks.reduce((acc, b) => {
      let m;
      if (b.meal === WEEKLY_TREAT_MEAL) {
        if (b.type === 'item') {
          m = treatMacroContrib(b.entry);
        } else {
          const contribs = b.entries.map(e => treatMacroContrib(e));
          const sum = contribs.reduce((s, c) => ({ kcal: s.kcal+c.kcal, p: s.p+c.p, c: s.c+c.c, f: s.f+c.f }), {kcal:0,p:0,c:0,f:0});
          m = b.isExploded ? sum : { kcal: sum.kcal/b.servings, p: sum.p/b.servings, c: sum.c/b.servings, f: sum.f/b.servings };
        }
      } else if (b.type === 'item') {
        const e = b.entry;
...
async function loadStats() {
  ...
  const dayData = dates.map(date => {
    ...
    const isExcluded = status === 'sick' || status === 'freeze' || status === 'vacation';
  ...
  const excludedDays = dayData.filter(d => d.status === 'freeze' || d.status === 'sick' || d.status === 'vacation').length;
  ...
  const hasVacation = dayData.some(d => d.status === 'vacation');
  ...
  document.getElementById('statsOverallAdh').innerHTML = `
    ...
    <div class="stats-summary-sub">${withAdh.length} of ${dates.length} days counted${excludedDays > 0 ? ` · ${excludedDays} excluded (sick/freeze/vacation)` : ''}</div>`;
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: ctx => {
          ...
          const statusLabel = d && d.status === 'freeze' ? ' · Freeze ❄' : d && d.status === 'sick' ? ' · Sick 🤒' : d && d.status === 'vacation' ? ' · Vacation 🏖' : d && jokerDates.has(d.date) ? ' · Weekly Treat ⭐' : d && mocDates.has(d.date) ? ' · Meal of Choice 🍽️' : '';
  ...
  if (statsPeriod === 'week') {
    ...
    dayData.forEach(d => {
      ...
      else if (status === 'vacation') bg = 'rgba(34,211,238,0.2)';
      ...
      if (status === 'freeze') {
        ...
        mainContent = `<i class="fas fa-umbrella-beach" style="font-size:1.05rem;color:rgba(34,211,238,.9)"></i>`;
      } else {
        mainContent = `<div style="font-family:'Bebas Neue',sans-serif;font-size:1.15rem;color:${a!==null?'#fff':'#444'}">${a !== null ? a+'%' : '–'}</div>`;
      }
      const tipText = status === 'freeze' ? `${d.date}: Freeze` : status === 'sick' ? `${d.date}: Sick` : status === 'vacation' ? `${d.date}: Vacation` : `${d.date}: ${a !== null ? a + '%' : 'N/A'}${isJoker ? ' · Weekly Treat ⭐' : isMoC ? ' · Meal of Choice 🍽️' : ''}`;
    ...
    dayData.forEach((d, i) => {
      ...
      else if (status === 'vacation') bg = 'rgba(34,211,238,0.25)';
      ...
      if (status === 'freeze') {
        ...
        cell.innerHTML = `<span style="display:inline-flex;background:rgba(34,211,238,.45);border-radius:3px;padding:2px 3px;line-height:1"><i class="fas fa-umbrella-beach" style="font-size:.5rem;color:#fff"></i></span>`;
      } else if (isJoker) {
...
(function(){
  ...
  function showTlContextMenu(chipEl) {
    ...
    if (isUnplanned) {
      ...
      sheet.innerHTML = `
        <div class="tl-ctx-title">${name}</div>
        ${bgBtn}
        <button class="tl-ctx-action" id="tlCtxEditAmt"><i class="fas fa-pen"></i> Edit Amount</button>
      ...
      overlay.addEventListener('click', e => { if (e.target === overlay) close(); });
      sheet.querySelector('.tl-ctx-cancel').addEventListener('click', close);
      sheet.querySelector('#tlCtxEditAmt').addEventListener('click', () => { close(); showAmountEditSheet(chipEl); });
  ...
  function showAmountEditSheet(chipEl) {
    const kind = chipEl.dataset.dragKind;
    const entryIds = chipEl.dataset.entryIds.split(',').map(s => s.trim()).filter(Boolean);
    ...
    const fmtNum = n => Number.isInteger(n) ? String(n) : parseFloat(n.toFixed(1)).toString();

    if (kind === 'item') {
      const entry = currentDayEntries.find(e => String(e.id) === entryIds[0]);
      if (!entry) return;
      const oldAmt = extractAmount(entry.item_name);
      const baseName = stripAmount(entry.item_name);
      const prefix = entry.item_name.slice(0, entry.item_name.length - baseName.length).trim();
      const unitMatch = prefix.match(/^[\d.,]+\s*(.*)$/);
      const unit = unitMatch ? unitMatch[1].trim() : '';
      const origM = { kcal: entry.kcal||0, p: parseFloat(entry.protein)||0, c: parseFloat(entry.carbs)||0, f: parseFloat(entry.fat)||0 };

      const useScale = oldAmt === 0;
      sheet.innerHTML = `
        <div class="tl-ctx-title"><i class="fas fa-pen" style="margin-right:6px;color:var(--accent)"></i>Edit Amount</div>
        <div class="tl-amt-name">${entry.item_name}</div>
        <div class="tl-amt-row">
          <label class="tl-amt-label">${useScale ? 'Scale' : 'Amount'}</label>
          <div class="tl-amt-input-wrap">
            <input type="number" id="tlAmtInput" class="tl-amt-input" value="${useScale ? '1' : fmtNum(oldAmt)}" min="0.01" step="${useScale ? '0.1' : 'any'}">
            <span class="tl-amt-unit">${useScale ? '×' : (unit || '')}</span>
          </div>
        </div>
        <div class="tl-amt-macros" id="tlAmtPreview">${pillsHTML(origM)}</div>
      ...
      input.addEventListener('input', () => {
        ...
        const scale = useScale ? val : val / oldAmt;
        preview.innerHTML = pillsHTML({ kcal: origM.kcal*scale, p: origM.p*scale, c: origM.c*scale, f: origM.f*scale });
      });
      ...
      sheet.querySelector('#tlAmtSave').addEventListener('click', async () => {
        ...
        const { error } = await db.from('fddb_daily_macros').update(updates).eq('id', entry.id);
        if (error) { showToast('Error saving amount', 'error'); return; }
